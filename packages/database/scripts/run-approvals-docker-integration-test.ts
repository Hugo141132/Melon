...
async function main() {
  ...
  const containerName = 'kebun_melon_test_approvals_' + nonce;

  const port = await getAvailablePort();
  const testDbUrl = `postgresql://${dbUser}:${dbPass}@127.0.0.1:${port}/${dbName}`;

  console.log('--- STARTING REPRODUCIBLE DOCKER APPROVALS INTEGRATION TEST WORKFLOW ---');
  ...
  try {
    ...
    console.log('--- REPRODUCIBLE DOCKER APPROVALS INTEGRATION TEST SUCCEEDED ---');
  } finally {
    // Guaranteed cleanup
