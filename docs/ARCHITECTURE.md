...
- **Singapore Dev Production-Equivalent Cutover:** Development persistence has cut over to Supabase PostgreSQL in AWS Singapore (`ap-southeast-1`, `unbyxlkrzqlafolxcypi`), co-locating persistence with EMQX Cloud broker (`asia-southeast1`). Web and Gateway connect to the transaction pooler (port 6543, `?pgbouncer=true`).
- **Pre-Write Manifest Parity & Migration History:** Restored with 100% bit-for-bit application table row parity against immutable `backups/cutover/dev_20260908_025808/dev_manifest.tsv` (which recorded 11 migration rows; deployment produced 13). Reconciled Prisma migration history: 13 total records (11 applied matching local repository checksums bit-for-bit + 2 historical rollbacks). Physically verified all 13 performance indexes (`indisvalid=true`, `indisready=true`).
- **Internal Service Token Rotation:** Rotated `INTERNAL_SERVICE_TOKEN` using 32-byte hex CSPRNG across `.env`, `apps/web/.env`, and `apps/iot-gateway/.env`. Machine-to-machine authentication between Web and Gateway verified for probes and realtime event publishing.
- **Authentication, Telemetry & Realtime Verification:** Genuine interactive Owner login verified (1 active session `de8a9c04-5829-44bb-875a-eca4edbf5a88` in `public.sessions`, single active session enforced). Telemetry ingestion persisted 3 synthetic soil readings (`soil_readings` count 0 $\rightarrow$ 3), atomically updated device `soil-node-jvbkdbv` timestamps, and verified subscriber-side receipt of `cutover-sse-subscriber-20260908-03` as `event: telemetry.soil.updated` via EventSource with stream closed cleanly. `public.faucet_commands` remains strictly 0 (`ENABLE_FAUCET_CONTROL=false`).
- **E2E Test Isolation Hardening:** Pinned Playwright to dedicated test port `3005` (`baseURL: http://localhost:3005`), `reuseExistingServer: false`, and enforced fail-closed `validateTestDatabaseUrl` guards preventing test pollution of Singapore Dev.
- **Staging & Mumbai Dev State:** Mumbai Dev (`xjsencdgfcbkzdzqcnqx`) is paused and permanently stale. Cloud Staging database (`scqrbtfilmttqrutynyo`) remains active on Mumbai and containerized staging services remain exited; Staging cutover, container redeployment, soak period, and retirement remain pending maintenance window. `TASK-0916` status remains `IN_PROGRESS`. Reference: `docs/DATABASE.md` §2.2, `docs/TESTING.md` §35, and [`docs/SUPABASE_MIGRATION_RUNBOOK.md`](file:///c:/Users/Puroh/Documents/Melon/docs/SUPABASE_MIGRATION_RUNBOOK.md).
