...
`TASK-0916` governs the database migration from AWS Mumbai (`ap-south-1`) to AWS Singapore (`ap-southeast-1`) under `DEC-INF-095`:
- **Singapore Dev Restoration & Baseline Fidelity:** Restored to Singapore Dev (`unbyxlkrzqlafolxcypi`, AWS `ap-southeast-1`) via containerized PostgreSQL 17 client with fail-closed preflight checks. Verified 100% bit-for-bit application table row parity across all 25 non-migration tables against the immutable cutover manifest [`backups/cutover/dev_20260908_025808/dev_manifest.tsv`](file:///c:/Users/Puroh/Documents/Melon/backups/cutover/dev_20260908_025808/dev_manifest.tsv) (which recorded 11 migration history rows; deployment produced 13), 28 foreign key constraints with 0 orphan rows, and 0 unresolved failures.
- **Singapore Dev Migration Deployment & Post-Cutover Mutations:** Deployed pending migrations `20260820000000` and `20260905040000` (13 rows in `_prisma_migrations`: 11 applied + 2 historical rollbacks; 13 performance indexes valid/ready). Mutations: 1 active Owner session (`de8a9c04-5829-44bb-875a-eca4edbf5a88`, `DEC-AUTH-107` enforced), 3 synthetic soil readings (`soil_readings` count 0 $\rightarrow$ 3), `soil-node-jvbkdbv` timestamps refreshed, `faucet_commands` strictly 0 (`ENABLE_FAUCET_CONTROL=false`).
- **Singapore Staging Restoration & Baseline Fidelity:** Restored to Singapore Staging (`ihgoxqdncepbcrqkchxu`, AWS Singapore `ap-southeast-1`, PostgreSQL 17.6.1.166). Verified 100% bit-for-bit row parity across all 26 public tables against immutable cutover manifest [`backups/cutover/staging_20260908_185145/staging_manifest.tsv`](file:///c:/Users/Puroh/Documents/Melon/backups/cutover/staging_20260908_185145/staging_manifest.tsv) (SHA-256 `BC03C209639942BEA678B1353D382C4E354C7A800082187E60807ADD8E43A9FB`), 28 foreign keys with 0 orphan rows, 10 baseline migrations, and platform event trigger `ensure_rls` active.
- **Singapore Staging Migration Deployment & History Reconciliation:** Deployed additive migration `20260905040000_add_auth_and_fk_performance_indexes` via `deploy_singapore_staging_migrations.ps1`, bypassing the Dev index-drop workaround and preserving existing valid `sessions_user_active_idx`. Produced exactly 11 applied rows in `_prisma_migrations` (0 unresolved failures, 0 rollbacks). Verified all 13 performance indexes valid (`indisvalid=true`) and ready (`indisready=true`).
- **Singapore Staging Post-Cutover Live Data Mutations (Separated from Baseline):**
  - `_prisma_migrations`: 10 $\rightarrow$ 11 (+1 deployed performance migration).
  - `sessions`: 55 $\rightarrow$ 56 (+1 active Owner session `3072c4f8-973e-4502-ab6b-8df589eaff72` created at `2026-09-08 12:42:31.978 UTC`, `DEC-AUTH-107` enforced).
  - `audit_logs`: 104 $\rightarrow$ 105 (+1 synchronous log `35c7c64b-73f1-4f37-bbfd-ed190d779c44` for event `auth.login.success`).
  - `soil_readings`: 0 $\rightarrow$ 3 (cutover `676f7aca`, pre-fix diagnostic `965f54cf`, and coordinated live `d319dd56-821c-47b8-a56e-4012cd26f4f4` at `2026-09-08 16:06:10.106 UTC`). Real-time browser EventSource receipt confirmed by operator with 100% field correlation (Gate 5 PASS).
  - Device Metadata: `soil-node-biuc2f` timestamps atomically refreshed to `2026-09-08 16:06:10.106 UTC`.
  - Command Isolation: `public.faucet_commands` remains strictly 0 (`ENABLE_FAUCET_CONTROL=false` strictly enforced).
  - All other 22 public tables: Exactly 0 delta (100% manifest match).
...
## External ML Prediction Database Boundary & Device Mappings (TASK-0413 / DEC-MON-090 / Reconciled 2026-09-18)

This section documents the external database boundary, persistence invariants, verified schema, and Melon's database-driven device mapping for machine learning predictions:

### 1. Database Tier Ownership & Invariants
- **Melon System of Record:**
  - Internal PostgreSQL databases in AWS Singapore (`unbyxlkrzqlafolxcypi` for Dev, `ihgoxqdncepbcrqkchxu` for Staging) persist all core application state: `users`, `user_roles`, `sessions`, `devices`, `device_assignments`, `device_external_mappings`, `soil_readings`, `water_readings`, `reservoir_water_readings`, `faucet_commands`, `alerts`, and `audit_logs`.
  - **Telemetry Isolation:** Raw telemetry tables (`soil_readings`, `water_readings`, `reservoir_water_readings`) remain strictly untouched and immutable.
  - **No Local Model/Inference Tables:** Prediction results are not stored in Melon database; Melon acts as an external consumer.
- **External ML Team Database:**
  - Owned and managed independently by the external ML team at Supabase project `https://styjuynxuykvujnnqxos.supabase.co`.
  - Stores prediction outputs generated by their offline/online ML inference pipeline.
  - Accessible strictly via read-only PostgREST HTTPS API using server-side keys (`EXTERNAL_ML_SUPABASE_URL`, `EXTERNAL_ML_SUPABASE_PUBLISHABLE_KEY` / `EXTERNAL_ML_SUPABASE_SECRET_KEY`).

### 2. Melon `device_external_mappings` Table (Migration `20260918190000_add_device_external_mappings`)
To eliminate hard-coded hardware aliases in production, Melon manages dynamic external device identity mappings:

| Column | PostgreSQL Type | Nullable | Default | Description |
|---|---|---|---|---|
| `id` | `UUID` | NO | `gen_random_uuid()` | Primary Key |
| `device_id` | `UUID` | NO | - | Foreign Key $\rightarrow$ `devices(id)` ON DELETE CASCADE |
| `provider` | `VARCHAR(50)` | NO | `'EXTERNAL_ML'` | External integration provider identifier |
| `domain` | `VARCHAR(50)` | NO | - | Monitoring domain (`SOIL`, `WATER`) |
| `external_device_id` | `VARCHAR(150)` | NO | - | Identifier in external provider system (e.g. `melon002`, `water001`) |
| `is_active` | `BOOLEAN` | NO | `true` | Active flag for soft disabling without deletion |
| `created_at` | `TIMESTAMPTZ` | NO | `now()` | Record creation timestamp |
| `updated_at` | `TIMESTAMPTZ` | NO | `now()` | Timestamp of last modification |

**Constraints & Indexes:**
- **Unique Constraint:** `(device_id, provider, domain)` via `device_external_mappings_device_provider_domain_key` (enforcing one active mapping per provider & domain per device).
- **Lookup Indexes:**
  - `device_external_mappings_device_id_idx` on `(device_id)`
  - `device_external_mappings_provider_domain_ext_id_idx` on `(provider, domain, external_device_id)`

### 3. Verified External ML Schema
Both `soil_predictions` and `water_predictions` tables have been physically verified against the live external database via PostgREST metadata:

#### `soil_predictions` Table
| Column | PostgreSQL Data Type | Nullable | Default | Description |
|---|---|---|---|---|
| `id` | `uuid` | NO | `extensions.uuid_generate_v4()` | Primary Key |
| `device_id` | `character varying` | YES | `NULL` | Identifier matching external mapped ID (e.g. `melon002`) |
| `classification` | `character varying` | YES | `NULL` | Condition label (`optimal`, `warning`, `kritis`) |
| `confidence` | `double precision` | YES | `NULL` | Prediction confidence score ($0.0 - 1.0$) |
| `recommendation` | `text` | YES | `NULL` | Serialized JSON string |
| `created_at` | `timestamp with time zone` | NO | `now()` | Record creation timestamp |

#### `water_predictions` Table
| Column | PostgreSQL Data Type | Nullable | Default | Description |
|---|---|---|---|---|
| `id` | `uuid` | NO | `extensions.uuid_generate_v4()` | Primary Key |
| `device_id` | `character varying` | YES | `NULL` | Identifier matching external mapped ID (e.g. `water001`) |
| `classification` | `character varying` | YES | `NULL` | Condition label (`optimal`, `warning`, `kritis`) |
| `confidence` | `double precision` | YES | `NULL` | Prediction confidence score ($0.0 - 1.0$) |
| `recommendation` | `text` | YES | `NULL` | Serialized JSON string |
| `created_at` | `timestamp with time zone` | NO | `now()` | Record creation timestamp |

### 4. Recommendation JSON Payload Structure
The `recommendation` text column stores a structured JSON document:
```json
{
  "module": "soil",
...
### 5. Device Resolution Flow
- **Production (`isProduction = true`):** Requires an active entry in `device_external_mappings`. If no mapping exists, queries fail closed safely without falling back to hardcoded strings.
- **Development / Test:** If unmapped, falls back to `DEFAULT_ML_DEVICE_ALIASES` (`soil-node-jvbkdbv` $\rightarrow$ `melon002`, `water-quality-node-quiua` $\rightarrow$ `water001`) for convenience.
<!-- External ML Database Reconciled: 2026-09-18 -->
