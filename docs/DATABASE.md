...
### 2.2 TASK-0916 Migration Preparation, Singapore Dev Cutover & Verification Reconciliation
...
- **Migration Deployment & History Reconciliation:** Executed `deploy_singapore_dev_migrations.ps1` with bounded lock timeout (`SET lock_timeout = '5s'`), applying pending migrations `20260820000000_add_session_user_active_index` and `20260905040000_add_auth_and_fk_performance_indexes`. Migration history contains exactly 13 rows in `_prisma_migrations` (11 applied matching repository SHA-256 checksums bit-for-bit + 2 historical rollbacks). Physically verified all 13 performance indexes (`indisvalid=true`, `indisready=true`).
- **Post-Cutover Live Data Mutations (Separated from Baseline):**
  - Sessions: Exactly 1 active Owner session (`de8a9c04-5829-44bb-875a-eca4edbf5a88` for `hugo@resend.dev`), single active session invariant (`DEC-AUTH-107`) enforced.
  - Telemetry: Exactly 3 synthetic soil readings (`soil_readings` count 0 $\rightarrow$ 3: `902f6f4e`, `681f5441`, and `04bafee1`).
  - Device Metadata: `soil-node-jvbkdbv` timestamps atomically refreshed to ingestion timestamps.
  - Command Isolation: `public.faucet_commands` strictly 0 (`ENABLE_FAUCET_CONTROL=false` strictly enforced).
- **Service Connectivity, Token Rotation & SSE Delivery:** Web and Gateway connected to Singapore Dev pooler (port 6543, `?pgbouncer=true`). Rotated `INTERNAL_SERVICE_TOKEN` using 32-byte CSPRNG. Ingested REST telemetry and verified subscriber-side receipt of `cutover-sse-subscriber-20260908-03` as `event: telemetry.soil.updated` via EventSource with stream closed cleanly.
- **E2E Isolation Hardening:** Pinned Playwright to dedicated test port `3005` (`baseURL: http://localhost:3005`), `reuseExistingServer: false`, and enforced fail-closed `validateTestDatabaseUrl` guards preventing test pollution of Singapore Dev.
- **Environment Status & Staleness:** Mumbai Dev (`xjsencdgfcbkzdzqcnqx`) is paused and permanently stale. Cloud Staging (`scqrbtfilmttqrutynyo`) and containerized staging services remain unaffected on Mumbai; Staging cutover, container redeployment, soak period, and retirement remain pending maintenance window. `TASK-0916` status remains `IN_PROGRESS`. Full execution evidence is maintained in [`docs/SUPABASE_MIGRATION_RUNBOOK.md`](file:///c:/Users/Puroh/Documents/Melon/docs/SUPABASE_MIGRATION_RUNBOOK.md).
