...
- **Singapore Dev Cutover & Verification:** Singapore Dev (`unbyxlkrzqlafolxcypi`) is verified with 100% table row parity against immutable snapshot manifest `backups/cutover/dev_20260908_025808/dev_manifest.tsv` (11 baseline migration rows, 13 post-deploy records: 11 applied + 2 historical rollbacks). All 13 performance indexes are verified. Internal service token rotated (32-byte CSPRNG). Genuine interactive Owner login verified (1 active session `de8a9c04-5829-44bb-875a-eca4edbf5a88`, single active session enforced). REST telemetry persisted (3 soil readings) and subscriber-side SSE chunk receipt verified. E2E test isolation is hardened with dedicated port `3005` and fail-closed database validation.
- **Singapore Staging Cutover & Verification:** Singapore Staging (`ihgoxqdncepbcrqkchxu`) cutover is verified with 100% table row parity against immutable snapshot manifest `backups/cutover/staging_20260908_185145/staging_manifest.tsv` (10 baseline migrations, 11 applied post-deploy with zero rollbacks/failures). All 13 performance indexes valid and ready. Containerized staging services (`melon-web` and `melon-iot-gateway`) redeployed and healthy. Interactive Owner login verified (active session `3072c4f8-973e-4502-ab6b-8df589eaff72`, synchronous audit log recorded, `DEC-AUTH-107` enforced) and telemetry writes to `soil_readings` (`676f7aca`, `965f54cf`, `d319dd56`). Remediated in-route event publishing in telemetry endpoints and verified live browser EventSource receipt (`telemetry.soil.updated` correlating 100% with reading `d319dd56-821c-47b8-a56e-4012cd26f4f4`, Gate 5 PASS). `ENABLE_FAUCET_CONTROL=false` strictly preserved.
- **Environment Status & Staleness:** Both Mumbai Dev (`xjsencdgfcbkzdzqcnqx`) and Mumbai Staging (`scqrbtfilmttqrutynyo`) are paused (`INACTIVE`, 0 active project slots) and permanently stale. Per Owner decision, Mumbai rollback is decommissioned and technical cutover is complete (`DONE`). Restarted 72-hour soak period active from `2026-09-08 15:28:30 UTC` through `2026-09-11 15:28:30 UTC`. Sampled observations confirm healthy service at tested points, with the gap between periodic sample probes and continuous telemetry aggregation explicitly disclosed. Full execution details are maintained in [`docs/SUPABASE_MIGRATION_RUNBOOK.md`](file:///c:/Users/Puroh/Documents/Melon/docs/SUPABASE_MIGRATION_RUNBOOK.md).
