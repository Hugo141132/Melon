...
The following security controls and cryptographic procedures were verified during the database migration, Singapore Dev cutover, and test isolation hardening for `TASK-0916`:
...
- **Fail-Closed Restore & Bounded Lock DDL:** Dev restore enforced a fail-closed preflight check ensuring the target is clean prior to decryption or DDL execution. Migration deployment wrapper `deploy_singapore_dev_migrations.ps1` applied bounded lock timeouts (`SET lock_timeout = '5s'`) without `CASCADE` to guarantee safe lock acquisition while writers were stopped.
- **Internal Service Token Rotation:** Rotated `INTERNAL_SERVICE_TOKEN` using 32-byte CSPRNG across `.env`, `apps/web/.env`, and `apps/iot-gateway/.env`. Verified machine-to-machine authentication: Gateway probe `/internal/v1/ready` rejects unauthorized/stale tokens (HTTP 401) and validates with the rotated token (HTTP 200); Web internal publish webhook (`POST /api/v1/internal/realtime/publish`) verifies Bearer authentication (HTTP 200).
- **Session Security & Single Active Session Invariant:** Genuine interactive Owner login verified. Active session confirmed in PostgreSQL catalog (`public.sessions` = 1 active session `de8a9c04-5829-44bb-875a-eca4edbf5a88` for `hugo@resend.dev`), strictly enforcing single active session invariant (`DEC-AUTH-107`).
- **Complete E2E Test Isolation Hardening:** Playwright test runner is pinned to dedicated port `3005` (`baseURL: http://localhost:3005`), `reuseExistingServer: false`, and enforces fail-closed `validateTestDatabaseUrl` guards across `playwright.config.ts`, `e2e/critical-flows.spec.ts`, and `packages/database/scripts/run-docker-integration-test.ts` to prevent accidental execution against Singapore Dev.
- **Environment Status & Staleness:** Mumbai Dev (`xjsencdgfcbkzdzqcnqx`) is paused and permanently stale. Staging cloud database (`scqrbtfilmttqrutynyo`) and containerized staging services remain unaffected on Mumbai pending scheduled downtime maintenance window. `TASK-0916` status remains `IN_PROGRESS`. Full execution evidence in [`docs/SUPABASE_MIGRATION_RUNBOOK.md`](file:///c:/Users/Puroh/Documents/Melon/docs/SUPABASE_MIGRATION_RUNBOOK.md).
