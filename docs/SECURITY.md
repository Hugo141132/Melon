...
- **Anti-Enumeration Baseline & DEC-AUTH-108 Approved Exception**: Under standard baseline policies, `POST /api/v1/auth/forgot-password` returned generic HTTP 200 responses. Per **`DEC-AUTH-108`**, an approved operational exception is implemented: if the submitted email does not exist in the database, the endpoint returns HTTP 404 with error code `EMAIL_NOT_FOUND` and message *"Alamat email tidak terdaftar dalam sistem kami."* to eliminate operational confusion across isolated development/staging environments. The trade-off is accepted for this agricultural operations system and mitigated by:
  - Strict IP rate limiting: `RATE_LIMIT_FORGOT_PASSWORD_MAX = 3` req/min per IP via `checkRateLimit`.
  - Structured audit logging (`auth.password_reset.failed`) capturing IP and User-Agent.
  - Reset status verification via `GET /api/v1/auth/forgot-password?email=...` (rate limited to 10 req/min per IP) allowing client status checks without exposing tokens or user records.
...
- **Approved Operational Policies**:
  - Token expiry is formally approved as **1 minute** (`AUTH_RESET_TOKEN_EXPIRY_MINUTES = 1`, `DEC-AUTH-109` / `TASK-0218`).
  ...
  - Forgot-password frontend resend cooldown is formally approved as **60 seconds (1 minute)** (`DEC-AUTH-109` / `TASK-0218`).
...
- **User Management Isolation for Unverified Accounts (DEC-AUTH-111)**: Accounts that have registered but not yet completed email verification (`emailVerifiedAt IS NULL`) MUST NOT appear in the Owner User Management dashboard (`UserRepository.getUsers`). Single-user inspection (`UserRepository.getUserManagementById`) returns `null` (HTTP 404 `USER_NOT_FOUND`) if the account is in `PENDING_APPROVAL` with `emailVerifiedAt === null`. Only verified accounts (`emailVerifiedAt IS NOT NULL`) with appropriate operational statuses (`PENDING_APPROVAL`, `ACTIVE`, `SUSPENDED`) are exposed, preventing premature administrative actions or data exposure. Client-side defensive filtering in `/users` enforces this invariant before rendering.
...
- **Code Expiry & Replay Protection**: Verification codes expire after **1 minute** (`AUTH_VERIFY_TOKEN_EXPIRY_MINUTES = 1`, `DEC-AUTH-109` / `TASK-0218`). Creating a new verification code transactionally invalidates prior unused codes for the user. Upon successful verification, the code record is consumed/deleted.
...
- **Delivery & Custom Sending Domain Scope**: Verification has been exercised using Resend test recipients and 6-digit code dispatch. In accordance with `DEC-AUTH-102`, the application configuration and email service have been reconciled to use the verified custom domain `Melon Madura <noreply@melonmadura.my.id>`. Live mailbox delivery to arbitrary external email recipients is subject to DNS verification of DKIM, SPF, and DMARC in the domain registrar's zone editor.
...
- **Token Cryptography & Scoping**: Generates a 6-digit numeric CSPRNG code (`100000`–`999999`) with 1-minute expiry (`AUTH_VERIFY_TOKEN_EXPIRY_MINUTES = 1`, `DEC-AUTH-109` / `TASK-0218`), persisting only user-and-target-scoped SHA-256 hashes `sha256(userId:newEmail:code)` in `email_verification_tokens`. Raw codes are never persisted or logged.
...
- **Anti-Enumeration Baseline & DEC-AUTH-108 Exception**: Under `DEC-AUTH-108`, `POST /api/v1/auth/forgot-password` returns HTTP 404 with error code `EMAIL_NOT_FOUND` if the email address is not registered in the database, avoiding confusing false positives during operations while mitigating enumeration via strict rate limiting (`RATE_LIMIT_FORGOT_PASSWORD_MAX = 3` req/min per IP) and structured audit logging.
...
- **Configurable Expiration**: Token expiry is environment-configurable via `AUTH_RESET_TOKEN_EXPIRY_MINUTES` (default: 1 minute per `DEC-AUTH-109`).
...
- **6-Digit Code Cryptography**: Verification codes are 6-digit numeric CSPRNG codes (`100000`–`999999`) with an approved 1-minute lifetime (`AUTH_VERIFY_TOKEN_EXPIRY_MINUTES = 1`, `DEC-AUTH-109` / `TASK-0218`).
...
- **Scoped Token Hashing & CSPRNG:** Verification codes are 6-digit numeric CSPRNG strings (originally 15-minute expiry, subsequently standardized to 1-minute expiry via `DEC-AUTH-109` / `TASK-0218`). Stored token hashes in PostgreSQL are scoped cryptographically via `sha256(userId:newEmail:code)`, preventing replay or cross-user token substitution.
...
## Database Migration Security, Singapore Dev Cutover & E2E Isolation Controls Note (TASK-0916 / Reconciled 2026-09-08)

The following security controls and cryptographic procedures were verified during the database migration, Singapore Dev cutover, Singapore Staging cutover, and test isolation hardening for `TASK-0916`:
- **Encrypted Backups & Cryptographic Verification:** All database data exports are encrypted symmetrically using AES-256 via GPG (`.sql.gpg`) with SHA-256 transport checksums. Passwords and passphrases are provided interactively via secure strings (`Read-Host -AsSecureString`), never stored on disk, passed as command-line flags, or logged in console output (`SEC-OPS-001`). Temporary plaintext data dumps are deterministically deleted inside `try ... finally` blocks (`Test-Path: False`).
- **Fail-Closed Restore & Bounded Lock DDL:** Both Dev and Staging restores enforced fail-closed target guards ensuring targets were clean prior to decryption or DDL execution. Atomic single-transaction restoration (`--single-transaction -v ON_ERROR_STOP=1`) was enforced. Migration deployment wrappers (`deploy_singapore_dev_migrations.ps1`, `deploy_singapore_staging_migrations.ps1`) applied bounded lock timeouts (`SET lock_timeout = '5s'`) without `CASCADE` to guarantee safe lock acquisition.
- **Internal Service Token Isolation:** Internal service tokens are strictly separated: Dev uses a rotated 32-byte hex CSPRNG token across local `.env` files; Staging uses an independent 48-byte token in `.env.staging`. Machine-to-machine authentication is enforced for Gateway readiness probes and Web realtime publishing (`POST /api/v1/internal/realtime/publish`) via Bearer token.
- **Session Security & Single Active Session Invariant:** Genuine interactive Owner logins verified on both Dev and Staging. Active sessions confirmed in PostgreSQL catalogs (`public.sessions`), strictly enforcing single active session invariant (`DEC-AUTH-107`) and synchronous audit logging (`auth.login.success`).
- **Complete E2E Test Isolation Hardening:** Playwright test runner is pinned to dedicated port `3005` (`baseURL: http://localhost:3005`), `reuseExistingServer: false`, and enforces fail-closed `validateTestDatabaseUrl` guards across `playwright.config.ts`, `e2e/critical-flows.spec.ts`, and `packages/database/scripts/run-docker-integration-test.ts` to prevent accidental execution against Singapore cloud databases.
