...
The Singapore Dev database cutover and internal service token rotation (`TASK-0916`) reinforce backend messaging integrity and regional infrastructure colocation:
- **Regional Colocation with EMQX Cloud:** Co-locating the database tier in AWS Singapore (`ap-southeast-1`) with the project's EMQX Cloud MQTT broker (`asia-southeast1`) reduces WAN round-trip latency for IoT Gateway persistence operations from ~240ms down to sub-50ms within the region.
- **Machine-to-Machine Token Rotation:** `INTERNAL_SERVICE_TOKEN` was securely rotated using a 32-byte CSPRNG across `.env`, `apps/web/.env`, and `apps/iot-gateway/.env`. Verified machine-to-machine authentication: Gateway probe `/internal/v1/ready` rejects unauthorized calls (HTTP 401) and validates with the rotated token (HTTP 200); Web internal publish webhook (`POST /api/v1/internal/realtime/publish`) verifies Bearer authentication (HTTP 200).
- **REST Telemetry Ingestion & Atomicity:** Verified real ingestion via `POST /api/v1/devices/soil-node-jvbkdbv/telemetry/soil` with 3 synthetic writes (`soil_readings` count 0 $\rightarrow$ 3: `902f6f4e`, `681f5441`, and `04bafee1`), atomically updating device `last_seen_at` and `last_message_at` timestamps.
- **Real-Time Subscriber SSE Verification:** Realtime event dispatch via internal webhook and authenticated EventSource stream delivery (`GET /api/v1/realtime/stream`) verified: uniquely identified payload `cutover-sse-subscriber-20260908-03` was received as `event: telemetry.soil.updated` on subscriber client matching database record `04bafee1-4ea7-4999-a223-2d497cbeafbc`; stream closed cleanly.
