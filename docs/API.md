...
### 2.2 TASK-0916 Database Relocation, Singapore Dev Cutover & Verification Reconciliation
...
- **Singapore Dev Cutover & Probe Verification:** Web (`http://localhost:3000`) and IoT Gateway (`http://localhost:3001`) successfully connect to Singapore Dev (`unbyxlkrzqlafolxcypi`) transaction pooler (port 6543). All standard health probes return HTTP 200 (`/health` and `/ready`).
- **Internal Service Token Rotation:** Rotated `INTERNAL_SERVICE_TOKEN` using 32-byte CSPRNG across `.env`, `apps/web/.env`, and `apps/iot-gateway/.env`. Machine-to-machine authentication is enforced and verified: Gateway `/internal/v1/ready` rejects missing/stale tokens (HTTP 401) and accepts new tokens (HTTP 200); Web internal publish webhook (`POST /api/v1/internal/realtime/publish`) verifies Bearer authentication (HTTP 200).
- **Authentication & Ingestion Verification:** Genuine interactive Owner login verified (`POST /api/v1/auth/login`). REST telemetry ingestion verified: `POST /api/v1/devices/soil-node-jvbkdbv/telemetry/soil` persisted 3 controlled synthetic readings (`soil_readings` count 0 $\rightarrow$ 3: `902f6f4e`, `681f5441`, and `04bafee1`) and atomically updated device `soil-node-jvbkdbv` timestamps.
- **Real-Time SSE Subscriber Verification:** Authenticated EventSource connection (`GET /api/v1/realtime/stream`) verified receiving `event: telemetry.soil.updated` matching payload `cutover-sse-subscriber-20260908-03` and the persisted Singapore database record; stream closed cleanly.
- **E2E Isolation Hardening:** Playwright test runner is pinned to dedicated port `3005` (`baseURL: http://localhost:3005`), `reuseExistingServer: false`, and enforces fail-closed `validateTestDatabaseUrl` guards preventing test pollution of Singapore Dev.
- **Staging & Mumbai Dev Status:** Mumbai Dev (`xjsencdgfcbkzdzqcnqx`) is paused and permanently stale. Cloud Staging (`scqrbtfilmttqrutynyo`) and containerized staging services remain unaffected on Mumbai pending scheduled downtime maintenance window. Detailed execution evidence is maintained in [`docs/SUPABASE_MIGRATION_RUNBOOK.md`](file:///c:/Users/Puroh/Documents/Melon/docs/SUPABASE_MIGRATION_RUNBOOK.md).
