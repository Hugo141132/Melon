...
- **Dev & Staging Cutover & Probe Verification:** Web (`http://localhost:3000`) and IoT Gateway (`http://localhost:3001`) connect to Singapore Dev (`unbyxlkrzqlafolxcypi`) and Singapore Staging (`ihgoxqdncepbcrqkchxu`) transaction poolers (port 6543). All health probes return HTTP 200 (`/health` and `/ready`).
- **Internal Service Token Isolation:** Machine-to-machine authentication is enforced and verified across environments: Gateway `/internal/v1/ready` and Web internal publish webhook (`POST /api/v1/internal/realtime/publish`) verify Bearer authentication (HTTP 200) using environment-segregated tokens (Dev: 32-byte CSPRNG; Staging: independent 48-byte token).
- **Authentication & Ingestion Verification:** Genuine interactive Owner logins verified (`POST /api/v1/auth/login`). REST telemetry ingestion verified: on Dev via `soil-node-jvbkdbv` (3 writes persisted to `soil_readings`), and on Staging via `soil-node-biuc2f` (Reading ID `676f7aca-a6f1-4ae4-a7ef-a00557a2c536` persisted to `soil_readings`), atomically updating device timestamps.
- **Real-Time SSE Subscriber Verification:** On Dev, authenticated EventSource connection (`GET /api/v1/realtime/stream`) verified receiving `event: telemetry.soil.updated` matching payload and DB record. On Staging, confirmed via live authenticated browser EventSource receipt (`event: telemetry.soil.updated`) correlating 100% with Reading ID `d319dd56-821c-47b8-a56e-4012cd26f4f4` (Gate 5 PASSED).
- **E2E Isolation Hardening:** Playwright test runner is pinned to dedicated port `3005` (`baseURL: http://localhost:3005`), `reuseExistingServer: false`, and enforces fail-closed `validateTestDatabaseUrl` guards preventing test pollution of Singapore cloud databases.
- **Environment Status & Staleness:** Both Mumbai Dev (`xjsencdgfcbkzdzqcnqx`) and Mumbai Staging (`scqrbtfilmttqrutynyo`) are paused (`INACTIVE`, 0 project slots) and permanently stale. Per Owner decision, Mumbai rollback is decommissioned and technical cutover is complete (`DONE`). Active 72-hour soak period restarted at `2026-09-08 15:28:30 UTC` following staging web redeployment (active through `2026-09-11 15:28:30 UTC`). Sampled observations confirm healthy service at tested points, with the gap between periodic sample probes and continuous telemetry aggregation explicitly disclosed. Detailed execution evidence is maintained in [`docs/SUPABASE_MIGRATION_RUNBOOK.md`](file:///c:/Users/Puroh/Documents/Melon/docs/SUPABASE_MIGRATION_RUNBOOK.md).
...
## Latest Machine Learning Prediction API Specification (TASK-0413 / DEC-MON-090 / Reconciled 2026-09-18)

This endpoint serves the most recent machine learning prediction and agronomic recommendation for a specified monitoring device:
...
- **Description:** Retrieves the latest soil or water quality classification, confidence score, diagnostic issues, and actionable recommendations generated by the external ML pipeline.
- **Authentication:** Required (User session cookie via `session_token`).
- **Authorization:** `requireDeviceViewAccess` (Caller must be `OWNER` or an `ADMIN` assigned to the target device; returns `403 Forbidden` otherwise).
- **Dual Identifier Resolution:** The `{deviceId}` route parameter accepts either the immutable database UUID (`devices.id`) or the canonical identifier string (`devices.device_id` / `devices.client_id`).
- **Dynamic Device Mapping:** Dynamically resolves active external device identifiers from Melon's database table `device_external_mappings` via `DeviceRepository.getActiveExternalDeviceId`. In production, devices without an active mapping fail closed safely. Aliases remain solely as a local dev/test fallback.
- **Caching:** In-memory cache-aside strategy with 30-second TTL. Cache hits return in $\le 10$ ms. Public requests consume cached results without exposing internal refresh controls.
- **Privacy & External Concealment:** Public responses mask external device IDs to canonical Melon `deviceId`, hide raw database fields, and completely omit `rawRecommendation`, external Supabase URLs, and credentials.

### 2. Response Payload Schemas

#### A. Soil Monitoring Device Response
```json
{
  "success": true,
  "data": {
    ...
    "issues": [],
    "modelVersion": null,
    "createdAt": "2026-09-18T00:33:06.431288+00:00"
  }
}
```

#### B. Water Quality Monitoring Device Response
```json
{
  "success": true,
  "data": {
    ...
    "modelVersion": null,
    "createdAt": "2026-09-18T01:43:44.798075+00:00"
  }
}
```

#### C. Safe Unavailable Response (No Prediction or Malformed Row)
```json
{
  "success": true,
  "data": null,
  "meta": {
    "requestId": "req-pred-latest-1773993600000",
    "status": "UNAVAILABLE",
    "message": "No prediction available for device 'soil-node-jvbkdbv'."
  }
}
```

#### D. Safe Unsupported Response (Device Type without ML Predictions)
```json
{
  "success": true,
  "data": null,
  "meta": {
    "requestId": "req-pred-latest-1773993600000",
    "status": "UNSUPPORTED",
    "message": "Predictions are not supported for device type 'WATER_TANK_NODE'."
  }
}
```

### 3. Error Responses
- `401 Unauthorized`: Missing or invalid session cookie (`UNAUTHENTICATED`).
