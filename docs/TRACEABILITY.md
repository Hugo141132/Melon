...
| `PRD-FR-041` | Latest external ML prediction ingestion and API access | `docs/PRD.md` | `DEC-MON-090` | `TASK-0413` | `TEST-API-003` | `IMPLEMENTED` |
| `DEV-MQTT-002` | Outbound AI recommendation MQTT publishing pipeline | `docs/DEVICE_COMMUNICATION.md` | `DEC-MON-090` | `TASK-0413` | `TEST-MQTT-001` | `IMPLEMENTED` |
| `SEC-DATA-005` | Database-driven external device mapping and fail-closed privacy isolation | `docs/SECURITY.md` | `DEC-MON-090` | `TASK-0413` | `TEST-DB-001` | `IMPLEMENTED` |
...
| `SEC-OPS-003` | Database backup encryption and offsite storage | `docs/SECURITY.md` | `DEC-INF-096` | `TASK-0909` | `TEST-SEC-005` | `DEFERRED` |
...
- **Profile UI Action Deduplication Refinement (2026-09-11):**
  - Traced to `DEC-UIUX-102` and `FLOW-AUTH-027` (`USER_FLOWS.md`).
  - Resolved duplicated "Change Email" action labels on `/profile`. Action button retained exclusively beside the Email field in personal info form; removed from Email Verification Status card in Account & Session Security.
  - Verified via unit test suite (`profile-page.test.tsx` 4/4 passed, asserting exactly 1 button in DOM for ID and EN), static typecheck (0 errors), containerized staging web rebuild (healthy on port 3000), and Playwright verification.
<!-- TASK-0216 and TASK-0217 Traceability Reconciled: 2026-08-30; Refined 2026-09-11 -->
...
## External ML Prediction & Outbound Recommendation Traceability Note (TASK-0413 / Reconciled 2026-09-18)

The following facts are verified in the traceability matrix regarding `TASK-0413` (External ML Prediction Ingestion and Outbound Recommendation Pipeline):
- **Traceability Baseline:** Governed by `PRD-FR-041`, `DEV-MQTT-002`, `SEC-DATA-005`, `DEC-MON-090`, `docs/DEVICE_COMMUNICATION.md` §53, `docs/API.md`, `docs/ARCHITECTURE.md`, `docs/DATABASE.md`, `docs/SECURITY.md`, and `docs/TESTING.md`. Formally establishes external Supabase PostgREST client adapter, protected prediction query API with RBAC, database-driven device mapping, and outbound asynchronous MQTT recommendation publishing with subscriber idempotency.
- **Implementation Status:**
  - *Phase A (Contracts & Client Adapter):* Canonical prediction contracts defined in `packages/contracts/src/prediction.ts` (`SoilPredictionDto`, `WaterPredictionDto`, `OutboundRecommendationPayload`, `PredictionIssue`). Read-only PostgREST client implemented in `packages/database/src/external-prediction-client.ts` with 30s TTL in-memory cache, 3000ms timeout protection, candidate column fallback, and JSON recommendation parsing.
  ...
  - *Phase D (Dashboard Recommendation Binding & Refinement):* `DONE` (Completed 2026-09-19; Refined 2026-09-24 — Bound live ML predictions and recommendations to `/soil` and `/water` via `useLatestPrediction` and reusable `RecommendationCard`. Refined to `Soft Bento Dashboard` direction inspired by `21st.dev` MCP with complete bilingual consistency across English and Indonesian via `recommendation-i18n.ts` helper and 20-key dictionary parity, while strictly preserving color tokens and mandatory advisory safety disclaimer per `DEC-UIUX-101`).
- **Verification Evidence:**
  - *Automated Unit & Route Test Suites (PASSED):*
    ...
    - `apps/web/test/unit/recommendation-card.test.tsx`: 9/9 passed (100% — including bilingual render tests).
    ...
    - Full Web Unit Test Suite: 87/87 test files passed, 730/730 tests passed (100%).
    - Monorepo Integration Test Suite: 129/129 test files passed, 1,328/1,328 tests passed (100%).
  - *Translation Completeness Check (PASSED):* `npm run i18n:check` verified 100% key parity across `messages/id.json` and `messages/en.json`.
...
<!-- TASK-0413 Traceability Reconciled: 2026-09-24 -->
