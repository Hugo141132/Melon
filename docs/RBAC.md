...
## Profile Security & Single Active Session RBAC Note (Reconciled 2026-08-29)

> **Associated Tasks:** `TASK-0217` (P0, DONE), `TASK-0216` (P1, READY)
> **Governing Decisions:** `DEC-AUTH-106`, `DEC-AUTH-107`, `DEC-UIUX-102`

The following role and authorization principles apply to verified self-email change and single-session enforcement:
- **Reuse of Approved Self-Profile Permissions:**
  - Password change from `/profile` consumes the existing backend endpoint `POST /api/v1/auth/change-password` guarded by `profilee.password.update.self` and `session.revoke.self`.
...
- **Authority & Account Status Invariants:**
  - Password updates and email updates require `accountStatus = ACTIVE`. Self-management actions are unavailable to `PENDING_APPROVAL`, `REJECTED`, `SUSPENDED`, or `DEACTIVATED` accounts.
  - Changing an email address or password never alters account roles (`OWNER` / `ADMIN`), approval history, or user device assignments (`user_device_access`).
