...
15. Operational database relocation, Singapore Dev cutover, and Singapore Staging cutover (`TASK-0916`) preserve all user-facing journeys, authentication flows, and authorization checks without modification. Genuine Owner login (`POST /api/v1/auth/login`), single active session enforcement (`DEC-AUTH-107`), and REST telemetry ingestion were verified on both Singapore Dev (`unbyxlkrzqlafolxcypi`) and Singapore Staging (`ihgoxqdncepbcrqkchxu`). Real-time SSE delivery (`telemetry.soil.updated`) was verified on Dev and confirmed via live authenticated browser EventSource receipt on Staging (Reading ID `d319dd56-821c-47b8-a56e-4012cd26f4f4`). Per Owner decision, Mumbai rollback is decommissioned and technical cutover is complete (`DONE`). Containerized staging services (`kebun-melon-staging-web` and `kebun-melon-staging-gateway`) are active and healthy, with Mumbai projects safely paused under a restarted 72-hour soak period active through `2026-09-11 15:28:30 UTC`. Full details in [`docs/SUPABASE_MIGRATION_RUNBOOK.md`](file:///c:/Users/Puroh/Documents/Melon/docs/SUPABASE_MIGRATION_RUNBOOK.md).
...
## Gateway Command Publishing User Flows Implementation Note (Reconciled 2026-08-20)

The following facts are verified in the end-to-end user flow implementations regarding `TASK-0804` (`CommandPublisher` in `@kebun-melon/iot-gateway`):
