...
- **Verified Email Change Security (`DEC-AUTH-106`):** Adheres strictly to `SEC-AUTH-006` and `SEC-AUTH-007`. Re-authenticates user via current password, generates 6-digit CSPRNG verification codes with 1-minute expiry per `DEC-AUTH-109` / `TASK-0218` (originally 15 minutes), stores tokens strictly as SHA-256 hashes, maintains authority of existing email until confirmed, and omits raw email strings from audit logs (`account.email.changed`).
