...
>
> **Reconciliation Note (TASK-0916):** `TASK-0916` (Supabase database migration and Singapore Dev cutover) is an infrastructure, persistence, and backend verification task (`Frontend impact: NONE`). Selected UI direction: `N/A`. Existing color template: `UNCHANGED`. Selected motion effects: `None`. 21st.dev MCP: `NOT REQUIRED`. All Next.js App Router frontend layouts, pages, components, and Tailwind styling are preserved with zero visual regressions. Genuine Owner login, REST telemetry ingestion, and subscriber-side SSE delivery (`telemetry.soil.updated`) were verified in the active browser session. E2E test isolation is pinned to dedicated port `3005` (`reuseExistingServer: false`). Documentation-only changes require no staging container redeploy. Full details in [`docs/SUPABASE_MIGRATION_RUNBOOK.md`](file:///c:/Users/Puroh/Documents/Melon/docs/SUPABASE_MIGRATION_RUNBOOK.md).
