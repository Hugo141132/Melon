...
>
> **Reconciliation Note (TASK-0916):** `TASK-0916` (Supabase database migration and Singapore Dev and Staging cutover) is an infrastructure, persistence, and backend verification task (`Frontend impact: NONE`). Selected UI direction: `N/A`. Existing color template: `UNCHANGED`. Selected motion effects: `None`. 21st.dev MCP: `NOT REQUIRED`. All Next.js App Router frontend layouts, pages, components, and Tailwind styling are preserved with zero visual regressions across Singapore Dev (`unbyxlkrzqlafolxcypi`) and Singapore Staging (`ihgoxqdncepbcrqkchxu`). Genuine Owner logins, REST telemetry ingestion, container health probes, and subscriber-side browser EventSource receipt (`telemetry.soil.updated` correlating 100% with persisted reading `d319dd56-821c-47b8-a56e-4012cd26f4f4`, Gate 5 PASS) were verified. Technical cutover is complete (`DONE`). E2E test isolation is pinned to dedicated port `3005` (`reuseExistingServer: false`). Documentation updates require no staging container redeploy. Full details in [`docs/SUPABASE_MIGRATION_RUNBOOK.md`](file:///c:/Users/Puroh/Documents/Melon/docs/SUPABASE_MIGRATION_RUNBOOK.md).
>
> **Reconciliation Note (TASK-0413 Phase D):** `TASK-0413 Phase D` (Dynamic Dashboard Recommendation Cards) binds external ML agronomic advice to `/soil` and `/water` (`Frontend impact: MINOR`). Selected UI direction: `Premium Minimal Ops`. Existing color template: `UNCHANGED`. Selected motion effects: `Skeleton loading`, `Card hover`, `Healthy status`, `Critical alert`. 21st.dev MCP: `NOT REQUIRED`. Added client polling hook `useLatestPrediction` (30s cadence, `activeDeviceIdRef` race-condition guard) and reusable component `RecommendationCard` (4 states: loading skeleton with `aria-busy="true"`, empty/unavailable, populated, and stale/offline banner). Enforced advisory-only safety disclaimer (`advisoryDisclaimer`), added `recommendation` translation namespace across `messages/id.json` and `messages/en.json` (14 keys, 100% parity), and added mobile clearance (`pb-24` on `/water`). Verified 100% test pass rate across 20 focused unit tests, full web workspace suite (83/83 files, 697/697 tests), and monorepo typecheck (0 errors across 4 workspaces).
>
> **Reconciliation Note (2026-09-20 Brand Assets & Next.js Cleanups):** Reconciled brand and institutional logos across auth pages (`/login`, `/register`, `/forgot-password`, `/reset-password`, `/verify-email`), `TopAppBar`, and `Sidebar` (`Frontend impact: MINOR`). Selected UI direction: `Premium Minimal Ops`. Existing color template: `UNCHANGED`. Selected motion effects: `Button hover`. 21st.dev MCP: `NOT REQUIRED`. Implemented transparent `logo1.webp` (institutional partner logo) for auth views and sidebar footer, and `logo2.webp` (KING Agro wisata emblem) for top bar trigger and sidebar drawer header. Removed redundant textual labels (`"Kebun Melon"` next to top-left emblem and `"Kebun Melon Monitoring System"` from sidebar footer). Configured Resend email templates with inline MIME CID attachments (`cid:logo1`). Cleaned Next.js dev console warnings by adding `data-scroll-behavior="smooth"` to `<html>` and `style={{ width: 'auto' }}` to responsive Next.js `<Image>` tags. Verified 0 typecheck errors and 100% unit test pass rate.
>
> **Reconciliation Note (2026-09-21 Frontend Branding Copy Cleanup & Role Label Standardization):** Streamlined customer-facing textual branding across authentication views, navigation shells, and settings (`Frontend impact: MINOR`). Selected UI direction: `Premium Minimal Ops`. Existing color template: `UNCHANGED`. Selected motion effects: `None`. 21st.dev MCP: `NOT REQUIRED`.
> - **Visible Brand Removal**: Eliminated customer-facing `"Kebun Melon"` text occurrences from registration strings (`auth.chooseRoleSubtitle`, `auth.firstOwnerDesc`, `auth.adminRegistrationDesc`), language switcher modal (`settings.languageModalDesc`), status guard footer (`apps/web/app/(auth)/status/page.tsx`), and settings application metadata (`apps/web/app/setting/page.tsx`).
> - **Role Label Normalization**: Removed redundant parenthetical `(Owner)` suffix from `apps/web/app/(auth)/register/register-view.tsx`, standardizing Owner selection card typography to exactly `"OWNER / PIC"` via `{tUsers('ownerRole')}` consistent with User Management (`/users`).
...
>
> **Reconciliation Note (2026-09-24 Registration Visual Integration & Browser Title Metadata):** Refined the registration page visual integration and standardized application browser tab title metadata (`Frontend impact: MINOR`). Selected UI direction: `Premium Minimal Ops`. Existing color template: `UNCHANGED`. Selected motion effects: `Button hover`. 21st.dev MCP: `VALIDATED & APPLIED` (Lightweight structural inspiration for clean border-anchored card hierarchy).
> - **Unified Card Hierarchy**: Replaced the disjointed floating `<header>` on `/register` (`apps/web/app/(auth)/register/register-view.tsx`) with an integrated top app header inside a solid, border-anchored card container (`bg-surface-container-lowest`, `border border-outline-variant/60`, `shadow-[0_4px_24px_rgba(0,0,0,0.06)]`, `rounded-2xl`). Unifies the back button, view title (`Account Registration`), step indicator, institutional partner logo (`/logo1.webp`), solid role choice cards with controlled green accents (`border-2 border-primary`), form fields (`h-[52px]`), and footer login link into one cohesive visual hierarchy.
> - **Prohibition of Glassmorphism**: Strictly avoided `backdrop-blur`, semi-transparent cards, and heavy raster assets. Surfaces remain 100% solid white (`#ffffff`).
> - **Calibrated Organic Mesh Background**: Updated `AppBackground.tsx` and mesh SVGs with a non-dominant, calm agricultural pastel palette (`#f3f7f0` to `#ddecd8`) with soft translucency (`opacity 0.15 - 0.35`) and center-focused radial luminance, ensuring high contrast, immediate legibility, and zero layout shift (CLS = 0).
> - **Browser Tab Title Standardization**: Updated root App Router metadata in `apps/web/app/layout.tsx` (`title.default` and `title.template`) from `"Kebun Melon - Smart Farming"` to `"Melon Governance"`, displaying `"Melon Governance"` on browser tabs for pages defaulting to root metadata.
> - **Invariants Preserved & Verification**: All authentication logic, route guards, validation schemas, and database operations remain 100% untouched. Verified with 0 TypeScript errors (`npm run typecheck:web`), 100% test pass rate in unit suites, and browser evaluation.
> - **Staging Impact**: Frontend/documentation only. No Supabase database migration, edge function deployment, or container update required.
...
| `/water` | `app/water/page.tsx` | Client Component | **Water Quality & Reservoir Monitoring (`/water`)**: Real-time water metrics (pH, TDS, EC in `µS/cm`), historical telemetry chart (`WaterNutrientChart`, `HistoricalChartControls`), reservoir tank volume (0 L–2200 L scale; flow rate deleted per `DEC-MON-089`), 3-phase faucet preset UI. |
