...
export async function POST(request: Request, { params }: { params: { userId: string } }) {
  ...
  try {
    ...
    requirePermission(session, 'account.suspend');
          ...
          return NextResponse.json(
            {
              ...
              error: {
                ...
                message: 'Invalid request payload for suspension.',
    ...
    const result = await userRepo.suspendUser({
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code: 'INTERNAL_ERROR',
          message: 'An unexpected internal error occurred while suspending user.',
        },
        meta: { requestId },
      },
      { status: 500 }
