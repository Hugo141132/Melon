 ...
 *
 * Path A: ESP32/NodeMCU Soil Monitoring Equipment transmits telemetry
...
export async function POST(request: Request, { params }: { params: { deviceId: string } }) {
  ...
  try {
    ...
    // 4. Validate Soil Telemetry Schema
    const parseResult = SoilTelemetryPayloadSchema.safeParse(body);
      ...
      return NextResponse.json(
        {
          ...
          error: {
            code: 'VALIDATION_ERROR',
            message: 'Soil telemetry payload schema validation failed.',
    ...
    const result = await telemetryRepo.ingestSoilReading({
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code: 'INTERNAL_ERROR',
          message: 'An unexpected error occurred during soil telemetry ingestion.',
        },
        meta: { requestId },
      },
      { status: 500 }
