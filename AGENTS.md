...
## TASK-0916 Governance, Rehearsal & Dev Cutover Implementation Record

`TASK-0916` database migration, local restore rehearsal, and Singapore Dev cutover record:
- **Status:** `IN_PROGRESS` (Dev cutover verified; pending Staging maintenance window, Staging migration, staging container redeployment, soak period, independent recovery backup, and Owner-authorized Mumbai retirement)
- **Frontend impact:** `NONE`
- **Selected UI direction:** `N/A`
- **Existing color template:** `UNCHANGED`
- **Selected motion effects:** `None`
- **21st.dev MCP:** `NOT REQUIRED`
- **Summary:** Executed Singapore Dev cutover and hardened E2E test isolation for the Supabase PostgreSQL database migration from AWS Mumbai (`ap-south-1`) to AWS Singapore (`ap-southeast-1`) governed by `DEC-INF-095` and [`docs/SUPABASE_MIGRATION_RUNBOOK.md`](file:///c:/Users/Puroh/Documents/Melon/docs/SUPABASE_MIGRATION_RUNBOOK.md).
  - **Singapore Dev Restoration & Baseline Fidelity:** Restored schema, data, and RLS to Singapore Dev (`unbyxlkrzqlafolxcypi`, AWS `ap-southeast-1`) using containerized PostgreSQL 17 client with fail-closed preflight checks. Verified 100% bit-for-bit row count parity across all 25 non-migration application tables against the immutable cutover manifest [`backups/cutover/dev_20260908_025808/dev_manifest.tsv`](file:///c:/Users/Puroh/Documents/Melon/backups/cutover/dev_20260908_025808/dev_manifest.tsv) (which recorded 11 migration history rows; deployment produced 13), 28 foreign key constraints with 0 orphan rows, and 0 unresolved failures.
  - **Target-Locked Migration Deployment:** Executed `deploy_singapore_dev_migrations.ps1` applying pending migrations `20260820000000_add_session_user_active_index` and `20260905040000_add_auth_and_fk_performance_indexes` with bounded lock timeout (`SET lock_timeout = '5s'`). Produced exactly 13 rows in `_prisma_migrations` (11 applied matching local repository SHA-256 checksums bit-for-bit + 2 historical rollbacks). Physically verified all 13 performance indexes (`indisvalid=true`, `indisready=true`).
  - **Internal Service Token Rotation:** Rotated `INTERNAL_SERVICE_TOKEN` using 32-byte hex CSPRNG across `.env`, `apps/web/.env`, and `apps/iot-gateway/.env` with zero credential printing. Verified machine-to-machine authentication: Gateway probe `/internal/v1/ready` rejects missing/stale tokens (HTTP 401) and accepts new token (HTTP 200); Web probe `/ready` returns HTTP 200 with gateway `up`.
  - **Owner Authentication & Single Session Invariant:** Genuine interactive Owner login verified; active session confirmed in database (`public.sessions` = 1 active session `de8a9c04-5829-44bb-875a-eca4edbf5a88` for `hugo@resend.dev`), single active session invariant (`DEC-AUTH-107`) enforced.
  - **Telemetry Persistence & Subscriber SSE:** Ingested 3 synthetic soil telemetry writes (`soil_readings` count 0 $\rightarrow$ 3: `902f6f4e`, `681f5441`, and `04bafee1`), atomically refreshed device `soil-node-jvbkdbv` timestamps, and verified subscriber-side receipt of uniquely identified payload `cutover-sse-subscriber-20260908-03` as `event: telemetry.soil.updated` matching payload and Singapore Dev DB record on authorized EventSource client with stream closed cleanly. `public.faucet_commands` remains strictly 0 (`ENABLE_FAUCET_CONTROL=false`).
  - **Complete E2E Test Isolation Hardening:** Pinned Playwright to dedicated test port `3005` (`baseURL: http://localhost:3005`), disabled server reuse (`reuseExistingServer: false`), and enforced fail-closed `validateTestDatabaseUrl` guards in `playwright.config.ts`, `e2e/critical-flows.spec.ts`, and `packages/database/scripts/run-docker-integration-test.ts` preventing test pollution of Singapore Dev.
  - **Environment Status & Staleness:** Mumbai Dev (`xjsencdgfcbkzdzqcnqx`) is paused and permanently stale. Cloud Staging database (`scqrbtfilmttqrutynyo`) remains active on Mumbai and containerized staging services remain exited; Staging migration, immediate container update/redeploy, soak period, and retirement remain pending maintenance window.
...
<!-- TASK-0916 Dev Cutover Reconciled: 2026-09-08 -->
