...
- Selected motion effects: `Modal`, `Button hover`
- 21st.dev MCP: `NOT REQUIRED`
- Summary: Self-service email change workflow for authenticated users governed by `DEC-AUTH-106`. Reuses existing 6-digit numeric CSPRNG code infrastructure with approved 15-minute expiry (`AUTH_VERIFY_TOKEN_EXPIRY_MINUTES = 15`) and scoped hashing `sha256(userId:newEmail:code)` stored in `email_verification_tokens.pending_email`. Guarantees 100% authority of existing email until verification is complete. Enforces candidate email uniqueness, rate limits (3 req/min request, 5 req/min verify), active session preservation without logout, and privacy-preserving audit logging (`account.email.changed` with non-sensitive metadata, omitting plaintext raw old/new emails).
