...
`TASK-0411` hardware MQTT topic reconciliation and gateway ingress mapping record:
- Status: `BLOCKED` (Awaiting official hardware team payload specification; topic naming RESOLVED)
- Priority: `P1`
- Frontend impact: `NONE`
- Selected UI direction: `N/A`
- Existing color template: `UNCHANGED`
- Selected motion effects: `None`
- 21st.dev MCP: `NOT REQUIRED`
- Summary: Reconciled external hardware MQTT parameters per user-approved decision `DEC-DEV-031`. Established permanent external topic names: `irigasi/melon/sensor/volume` (tank water volume telemetry), `irigasi/melon/kontrol/valve` (valve OPEN/CLOSE behavioral control), and `irigasi/melon/setting/otomasi` (irrigation). Topic naming and trailing-whitespace questions are authoritatively RESOLVED with strictly zero whitespace. Maintained a persistent gateway ingress mapping boundary (`apps/iot-gateway/src/mqtt/hardware-reconciliation.ts`) translating flat external topics to the canonical internal namespace (`agriculture/{env}/{siteId}/{deviceId}/...`). Disallowed silent whitespace trimming and alternate subscriptions. Preserved canonical valve command rules where `OPEN`/`CLOSE` omit `targetVolumeMl`, `phase`, and `plantCount`. Clarified that MQTT topics require no advance creation on EMQX Cloud, while broker ACLs/subscriptions/routing readiness remain unconfirmed. Verified 74 focused tests passing (28 reconciliation + 46 gateway connectivity/routing/telemetry), 0 TypeScript errors across 4 packages, clean candidate build, and staging container/image parity (`melon-iot-gateway:latest`, healthy, `/ready` UP, `ENABLE_FAUCET_CONTROL=false`). Five operator CI gates remain pending; unrelated `client.ts` remains excluded.
