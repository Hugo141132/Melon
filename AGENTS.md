...
- 2026-09-05 Login Performance Optimization & Auth Bug Fixes Record:
  ...
  - Summary: Optimized login transaction latency and resolved authentication regressions.
    ...
    - Implemented Optimizations: (1) Enabled Prisma `previewFeatures = ["relationJoins"]` and added `relationLoadStrategy: 'join'` in `prisma.user.findUnique`, collapsing 3 sequential WAN round trips into 1 single SQL `LEFT JOIN` (saving ~1,400ms); (2) Streamlined session transaction by querying unrevoked sessions with `tx.session.findMany`, skipping cleanup writes for clean accounts (saving ~400ms); (3) Moved non-critical `user.update({ lastLoginAt })` outside the critical login transaction to run non-blockingly with `.catch(...)` error logging (saving ~400ms); (4) Preserved synchronous `tx.auditLog.create` inside the interactive transaction to ensure transactional durability; (5) Expected `POST /api/v1/auth/login` latency reduced from ~3.6–4.2s to ~1.2–1.6s (~60–70% latency reduction).
